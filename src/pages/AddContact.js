...
const AddContact = () => {
  ...
  const imagePicker = async e => {
    ...
    try{
            ...
            snapshot => {
                ...
                switch(snapshot.state){
                    ...
                    case firebase.storage.TaskEvent.RUNNING:
                        setIsUploading(false)
                        console.log("Uploading is is in Progress..")
                    break;
                }
                console.log(progress);
                if(progress === 100){ console.log("If");
                    setIsUploading(false)
                    toast("Uploaded", {type: "success"})
                }
            },
            error => {
                toast("some thing is wrong in state change", {type: "error"})
            },
            () => {
        ...
        console.error(error)
        toast("Something wents Wrong", {type: "error"})
    }
  };
  ...
  const addContact = async () => {
    ...
    try{
        ...
        .ref('contacts/' + v4())
        .set({
            name, email, phoneNumber, address, picture: downloadUrl, star, createdAt: curTime
        })
    }catch(error){
        console.error(error)
    }
